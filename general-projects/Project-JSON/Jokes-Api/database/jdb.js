const JokeDatabase = {
  ...
  getFilteredJokes(jokeType) {
    return this.jokes.filter((joke) => joke.jokeType === jokeType);
  },
  getIdOfLast() {
    return this.jokes.length > 0 ? this.jokes[this.jokes.length - 1].id : 0;
  },
  replaceJoke(id, updatedJokeText, updatedJokeType) {
    const jokeToUpdate = this.getJokeById(id);
    if (jokeToUpdate) {
      jokeToUpdate.jokeText = updatedJokeText;
      jokeToUpdate.jokeType = updatedJokeType;
    }
    return jokeToUpdate;
  },
  patchJoke(id, jokeData) {
    const jokeToUpdate = this.getJokeById(id);
    if (jokeToUpdate) {
      jokeData.jokeText ? (jokeToUpdate.jokeText = jokeData.jokeText) : '';
      jokeData.jokeType ? (jokeToUpdate.jokeType = jokeData.jokeType) : '';
    }
    return jokeToUpdate;
  },
  deleteJoke(id) {
    const indexToDelete = this.jokes.findIndex((joke) => joke.id === id);
    if (indexToDelete !== -1) {
...
function newJoke(text, type) {
  let id = JokeDatabase.getIdOfLast();
  id++;
  this.id = id;
  this.jokeText = text;
  this.jokeType = type;
  return JokeDatabase.saveJoke(this);
}

export { JokeDatabase, newJoke };
